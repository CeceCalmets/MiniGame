...
function circleTriangleCollision(circle, triangle) {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        let closetX = start.x + dot * dx
        let closetY = start.y + dot * dy

        if (!isPointOnLineSegment(closetX, closetY, start, end)) {
            closetX = closetX < start.x ? start.x : end.x
            closetY = closetY < start.y ? start.y : end.y
        }

        dx = closetX - circle.position.x
        dy = closetY - circle.position.y
